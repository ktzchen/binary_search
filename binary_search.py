...
def count_repeats(xs, x):
    ...
    def one(left1, right1):
        ...
        if xs[mid1] == x:
            right1 = mid1
        return one(left1, right1)

    def two(left2, right2):
        mid2 = (left2 + right2) // 2
        if left2 == right2:
            if xs[left2] < x:
                return left2
        ...
        if xs[mid2] == x:
            left2 = mid2 + 1
        return two(left2, right2)

    if xs == []:
        return 0

    larger_index = two(0, len(xs) - 1)
    smaller_index = one(0, len(xs) - 1)
    if smaller_index is None or larger_index is None:
        return 0
    else:
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    # if (hi-lo) < epsilon:
    #    return hi # within episilon or within lo-hi
    # else:
    #    m1 = lo + (hi - lo) / 2
    #    m2 = lo + (hi  - lo) / 2
    #    if f(lo) > f(m1):
